...
def compute_score(player: Player):
    ...
    player.svo_mean_self = float(np.mean(values_player))
    player.svo_mean_other = float(np.mean(values_other))

    angle_rad = np.arctan((player.svo_mean_other - 50) / (player.svo_mean_self - 50))
    angle_deg = float(np.degrees(angle_rad))
    player.svo_score = round(angle_deg, 3)
...
def compute_payoff(player: Player):
    ...
    # ✅ Retrieve total payoff from previous tasks
    previous_total_payoff = player.participant.vars.get("total_payoff", 0)

    # ✅ Update total payoff by adding the current task's payoff
    new_total_payoff = previous_total_payoff + float(player.payoff)
    player.participant.vars["total_payoff"] = new_total_payoff
