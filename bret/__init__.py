...
class Player(BasePlayer):
    ...
    def bomb(self):
        """Calculate whether the player explodes and update payoffs."""
        ...
        # ✅ Retrieve the total payoff from previous tasks
        previous_total_payoff = self.participant.vars.get("total_payoff", 0)

        # ✅ Update cumulative total payoff
        new_total_payoff = previous_total_payoff + float(self.payoff)
        ...
        txt_final = (f"Vous avez ouvert {self.n_boxes} boîtes. La bombe était dans la boîte {self.bomb_box}. "
                     f"Votre gain est de {self.payoff}. Votre total cumulé sur toutes les tâches est désormais {new_total_payoff}.")
...
class Decision(MyPage):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        # ✅ Compute the BRET payoff and update total payoff
        player.bomb()
